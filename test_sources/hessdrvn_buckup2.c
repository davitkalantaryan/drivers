...
static int s_nChannelNumber = 0;
module_param_named(channel, s_nChannelNumber, int, S_IRUGO | S_IWUSR);
...
static int __init hess_init(void)
{
	...
	chan = dma_request_channel(mask, filter2222, (void*)0);
	...
	//dmaSrc = SMC_MEM_START + BASE_NECTAR_ADC ;

	dmaSrc = SMC_MEM_START + BASE_NECTAR_ADC + OFFS_EVENT_FIFO;
	printk(KERN_ALERT "SMC_MEM_START + BASE_NECTAR_ADC + OFFS_EVENT_FIFO                   = %lu", dmaSrc);
	//dmaSrc = virt_to_bus((char*)mappedIOMem + BASE_NECTAR_ADC + OFFS_EVENT_FIFO) ;
	//printk(KERN_ALERT "virt_to_bus((char*)mappedIOMem + BASE_NECTAR_ADC + OFFS_EVENT_FIFO) = %lu",dmaSrc);
	//goto errIOunmap;
...
#endif
	pDst = dma_alloc_coherent(chan->device->dev, DMA_TEST_SIZE, &dmaDst, GFP_DMA | GFP_KERNEL);
	if (!pDst)
	{
		nRet = -3;
		goto errIOunmap;
	...
	s_pAtSlave->cfg = ATC_SRC_REP | ATC_DST_REP;
	s_pAtSlave->ctrla = ATC_SCSIZE_4 | ATC_DCSIZE_4;
...
errDmaUnmapSingle:
	...
	kfree(pDst);*/
	PRINT_MEMORY(pDst, 16);
	dma_free_coherent(chan->device->dev, DMA_TEST_SIZE, pDst, dmaDst);
#ifdef REAL_DEVICE
errIOunmap :
	iounmap(mappedIOMem);
errRelMemRegion:
