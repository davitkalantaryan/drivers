...
/*static*/ bool filter2222(struct dma_chan *chan, void *param)
{
	...
	case 0:
		...
		return dma_has_cap(DMA_SLAVE, chan->device->cap_mask) ? true : false;
...
static struct dma_slave_config s_Config = {
...
static int __init hess_init(void)
{
	...
	dma_cap_mask_t mask;
	struct dma_chan *chan = (struct dma_chan *) - 1;
	struct dma_chan *vChans[32];
	int i = 0, j;
	//int err = 0;
	//char *pSrc;
	char *pDst;
	...
	printk(KERN_ALERT "41;  MAX_DMA_ADDRESS = 0x%x\n", (unsigned int)MAX_DMA_ADDRESS);
	...
	while (i < 16)
	{
		...
		printk(KERN_ALERT "%.2d -> chan = %p\n", i, chan);
		if (!chan)
		{
			printk(KERN_ERR "No chan!!!\n");
			//return -4;
			break;
		}
		printk(KERN_ALERT "dma_has_capabilities[] = [");
		for (j = 0; j < DMA_TX_TYPE_END - 1; ++j)
		{
			printk(KERN_CONT "%d, ", dma_has_cap(j, chan->device->cap_mask));
		}
		printk(KERN_CONT "%d]\n", dma_has_cap(DMA_TX_TYPE_END - 1, chan->device->cap_mask));
		vChans[i++] = chan;
		//++i;
	}
	...
	if (vChans[0])
	{
		printk(KERN_ALERT "vChans[0]->device->device_prep_slave_sg = %p\n", vChans[0]->device->device_prep_slave_sg);
	}

	for (j = 0; j < i; ++j)
	{
		dma_release_channel(vChans[j]);
	}
	return -1;

	//chan = dma_request_channel(mask, NULL, NULL);
	//chan = dma_request_channel(mask, filter, (void*)1);
	...
	printk(KERN_ALERT "driver_string = %s\n", dev_driver_string(chan->device->dev));
	goto error4;
	...
	//printk(KERN_ALERT "dst[0-7] = [%d %d %d %d %d %d %d %d]", (int)pDst[0], (int)pDst[1], (int)pDst[2], (int)pDst[3],
	//	(int)pDst[4], (int)pDst[5], (int)pDst[6], (int)pDst[7]);

	PRINT_MEMORY(pDst, 16);
	//goto error2;
	//dmaSrc = dma_map_single(chan->device->dev, pSrc, DMA_TEST_SIZE, DMA_TO_DEVICE);
	//dmaSrc = dma_map_single(chan->device->dev, (char*)mappedIOMem+BASE_NECTAR_ADC + OFFS_EVENT_FIFO, DMA_TEST_SIZE, DMA_TO_DEVICE);
	...
	/*aConfig.direction = DMA_DEV_TO_MEM;
	...
	s_Config.src_addr = dmaSrc;
	s_Config.dst_addr = dmaDst;
	dmaengine_slave_config(chan, &s_Config);

	//goto error1;

	//tx = dmaengine_prep_dma_cyclic(chan, dmaDest, 2, 2, DMA_DEV_TO_MEM);
	//tx = chan->device->device_prep_dma_cyclic(chan, dmaDst, 2, 2, DMA_FROM_DEVICE);
	//printk(KERN_ALERT "tx = %p\n",tx);
	printk(KERN_ALERT "chan->device->device_prep_dma_cyclic = %p\n", chan->device->device_prep_dma_cyclic);
	goto error1;

	if (!tx)
	{
		nRet = -5;
		printk(KERN_ERR "prem_dma_memcpy error!\n");
		goto error1;
	}
...
error1:
	...
	//kfree(pSrc);
	iounmap(mappedIOMem);
	//release_region(SMC_MEM_START, SMC_MEM_LEN);
	//release_mem_region(SMC_MEM_START, SMC_MEM_LEN);
	release_mem_region(SMC_MEM_START, SMC_MEM_LEN);
error4:
	dma_release_channel(chan);

	//dma_fops.enable = NULL;
	//isa_init_dma();

	return nRet;

#ifdef CONFIG_NEED_DMA_MAP_STATE
	printk(KERN_ALERT "CONFIG_NEED_DMA_MAP_STATE\n");
#else
	printk("Not defined!\n");
#endif
...
#elif defined(_M68K_DMA_H)
	printk(KERN_ALERT "_M68K_DMA_H\n");
#elif defined(_ASM_DMA_H)
	printk(KERN_ALERT "_ASM_DMA_H\n");
#elif defined(__ASM_AVR32_DMA_H)
	printk(KERN_ALERT "__ASM_AVR32_DMA_H\n");
#elif defined(_BLACKFIN_DMA_H_)
	printk(KERN_ALERT "_BLACKFIN_DMA_H_\n");
#elif defined(_H8300_DMA_H)
	printk(KERN_ALERT "_H8300_DMA_H\n");
	//#elif defined(ENODEV)
	//	printk(KERN_ALERT "ENODEV\n");
#elif defined(_ASM_IA64_DMA_H)
	printk(KERN_ALERT "_ASM_IA64_DMA_H\n");
#elif defined(_ASM_M32R_DMA_H)
	printk(KERN_ALERT "_ASM_M32R_DMA_H\n");
#elif defined(_ASM_MICROBLAZE_DMA_H)
	printk(KERN_ALERT "_ASM_MICROBLAZE_DMA_H\n");
#elif defined(_ASM_POWERPC_DMA_H)
	printk(KERN_ALERT "_ASM_POWERPC_DMA_H\n");
#elif defined(_ASM_SCORE_DMA_H)
	printk(KERN_ALERT "_ASM_SCORE_DMA_H\n");
#elif defined(__ASM_SH_DMA_H)
	printk(KERN_ALERT "__ASM_SH_DMA_H\n");
#elif defined(_ASM_SPARC_DMA_H)
	printk(KERN_ALERT "_ASM_SPARC_DMA_H\n");
#elif defined(_ASM_TILE_DMA_H)
	printk(KERN_ALERT "_ASM_TILE_DMA_H\n");
#elif defined(__UM_DMA_H)
	printk(KERN_ALERT "__UM_DMA_H\n");
#elif defined(__UNICORE_DMA_H__)
	printk(KERN_ALERT "__UNICORE_DMA_H__\n");
#elif defined(_ASM_X86_DMA_H)
	printk(KERN_ALERT "_ASM_X86_DMA_H\n");
#elif defined(_XTENSA_DMA_H)
	printk(KERN_ALERT "_XTENSA_DMA_H\n");
#elif defined(__ASM_RC32434_DMA_H)
	printk(KERN_ALERT "__ASM_RC32434_DMA_H\n");
#elif defined(__ASM_ARCH_DMA_H)
	printk(KERN_ALERT "__ASM_ARCH_DMA_H\n");
#elif defined(__ASM_ARCH_MXC_DMA_H__)
	printk(KERN_ALERT "__ASM_ARCH_MXC_DMA_H__\n");
#elif defined(__MACH_DMA_H)
	printk(KERN_ALERT "__MACH_DMA_H\n");
#elif defined(__ASM_MACH_DMA_H)
	printk(KERN_ALERT "__ASM_MACH_DMA_H\n");
#else
	//printk(KERN_ALERT "still doesn't found S3C2410_RES_OK = %d!\n", S3C2410_RES_OK);
	printk(KERN_ALERT "still doesn't found!\n");
#endif
	return -1;
#endif  /* #if 1 */
