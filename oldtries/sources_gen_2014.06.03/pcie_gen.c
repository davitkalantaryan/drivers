...
long  pcie_gen_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case PCIE_GEN_GAIN_ACCESS:
		...
		aActiveDev.dev_pars.vendor_id = g_vPci_gens[tmp_slot_num].vendor_id;
		aActiveDev.dev_pars.device_id = g_vPci_gens[tmp_slot_num].device_id;
		aActiveDev.dev_pars.slot_num = g_vPci_gens[tmp_slot_num].m_PcieGen.m_slot_num;
...
void __exit pcie_gen_cleanup_module(void)
{
	...
	PRINT_KERNEL_MSG(KERN_WARNING,"pcie_gen_cleanup_module CALLED\n");
...
int __init pcie_gen_init_module(void)
{
	...
	PRINT_KERN_ALERT("Init module called\n");
	...
	if (result)
	{
		PRINT_KERNEL_MSG(KERN_NOTICE, "Error %d adding devno%d num%d", result, devno, 0);
		return result;
	}
	...
	/*&g_pcie_gen.tamc200_pci_dev->dev*/NULL, "pcie_gen_init");
	...
	for( i = 0; i < PCIE_GEN_NR_DEVS; ++i )
	{
		...
		if (result)
		{
			PRINT_KERNEL_MSG(KERN_NOTICE, "Error %d adding devno%d num%d", result, devno, i);
			return result;
		}
