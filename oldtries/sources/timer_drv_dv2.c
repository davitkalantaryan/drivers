...
struct SSingleDev	g_pSingDevices[MAX_BOARDS_NUMB];



static struct pciedev_dev2 g_pciedev_dev = {

	.m_nRegionAllocated = 0,
	.m_nMajor = 0,
	.m_cnFirstMinor = 0,
	.m_nNDevices = 0,
	.m_cnTotalNumber = MAX_BOARDS_NUMB,
	.m_pDevices = g_pSingDevices,
};
...
static int __devinit pciedev_probe(struct pci_dev *dev, const struct pci_device_id *id)
{
	...
	result = register_pcie_device_exp(dev, &pciedev_fops, &g_pciedev_dev, DEVNAME);
...
static int __init pciedev_init_module(void)
{
	...
	while( pPCI )
	{
		...
		{
			printk(KERN_ALERT "PCIEDEV_PROBE CALLED \n");
			result = register_pcie_device_exp(pPCI, &pciedev_fops, &g_pciedev_dev, DEVNAME);
			tmp_brd_num = 0;
			printk(KERN_ALERT "PCIEDEV_PROBE_EXP CALLED  FOR BOARD %i\n", tmp_brd_num);
