...
#define FPGA_SPI_PIN_OUT 2 // 1=="prototype v1"; 2=="prototype v2"
 ...
 #define FPGA_PROGRAMMING_TEXT "SPI pin out for prototype v1!"
 ...
 #define FPGA_PROGRAMMING_TEXT "SPI pin out for prototype v2!"
...
{
	setTestPin(FPGA_CLK,CLR);
	setTestPin(FPGA_MOSI,CLR);
	setTestPin(FPGA_CLK,SET);
}
inline void send1(void)
{
	setTestPin(FPGA_CLK,CLR);
	setTestPin(FPGA_MOSI,SET);
	setTestPin(FPGA_CLK,SET);
}
...
{
	setTestPin(FPGA_nCONFIG,CLR);
	udelay(TCFG_US);
	setTestPin(FPGA_nCONFIG,SET);
...
static int fpga_chrdrv_open(struct inode * node, struct file *f)
{
	...
	dev = container_of(node->i_cdev, hess_dev_t, fpga_driver.cdev);
	f->private_data = dev;
...
static int fpga_chrdrv_create(hess_dev_t* mydev)
{
	int err = 0;
	char name[30];
	chrdrv_t* chrdrv = &mydev->fpga_driver;
...
// Initialization of the Character Device
static void fpga_chrdrv_destroy(hess_dev_t* mydev)
{
	chrdrv_t* chrdrv = &mydev->fpga_driver;
