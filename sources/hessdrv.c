...
enum _DMA_TYPES_{ _NO_DMA_ = 0, _SLOW_DMA_, _FAST_DMA_, _VERY_FAST_DMA_ };
static int s_nDMAtype = 1;
...
static hess_dev_t my_device_instance;
...
typedef struct
{
	...
	hess_ring_buffer_t* fifo_scroll_ptr;
	void*				m_pDMAengine;
	dma_addr_t			m_busAddrInMemory;
	dma_addr_t			m_busAddrInCurrent;
	dma_addr_t			m_busfifo_scroll;
	hess1u_system_enum  m_hess1u_device_type;
...
#include "../dma_engine_driver/dma_engine_header.h"

static void dmaCallback(void* a_data)
{

	IRQDevice_t* dev = (IRQDevice_t*)a_data;

	switch (dev->m_hess1u_device_type)
	{
...
typedef int(*TypeRead)(void*,int,dma_addr_t,dma_addr_t,dma_addr_t,dma_addr_t,
	dma_addr_t, dma_addr_t, size_t, size_t, size_t, void*, TYPE_CALLBACK);

static inline irqreturn_t srqIrqHandlerDMAfastORslow(int irq_number, void *p_dev,TypeRead a_asyncDMAread)
{
	static int snFirst = 1;
	IRQDevice_t* dev = (IRQDevice_t*)p_dev;
	uint16_t words = 0;
	uint16_t numberOfEventsInFifo = 0;
	//SSingleTransfer vTransferInfo[2];

	dev->m_hess1u_device_type = HESS1U_SYSTEM_DRAWER;

	if (snFirst)
	{
		dev->m_hess1u_device_type = get_hess1u_device_type();
		DBG("hess1u_device_type == %s\n", get_hess1u_device_type_name(dev->m_hess1u_device_type));
		snFirst = 0;
	}
	else
	{
		hess1u_system_enum temp = get_hess1u_device_type();
		if (temp != dev->m_hess1u_device_type)
		{
			dev->m_hess1u_device_type = temp;
			DBG("hess1u_device_type == %s\n", get_hess1u_device_type_name(dev->m_hess1u_device_type));
		}
	}
...
#if 1
		(*a_asyncDMAread)(dev->m_pDMAengine, words, 
			dev->m_busAddrInCurrent,dev->m_busAddrInMemory,dev->m_busAddrInMemory + sizeof(hess_ring_buffer_t),
			dev->m_busfifo_scroll,
			SMC_MEM_START + BASE_HESS1U_DRAWER_READOUT + OFFS_HESS1U_DRAWER_READOUT_EVENTFIFO,
			SMC_MEM_START + BASE_HESS1U_DRAWER_READOUT + OFFS_HESS1U_DRAWER_READOUT_EVENTFIFOREADSTROBE,
			HESS_SAMPLE_DATA_SIZE, sizeof(hess_sample_data_debug_t),2,dev, dmaCallback);
#else
		(*a_asyncDMAread)(dev->m_pDMAengine, words, g_srqDevice.m_busAddrInCurrent, g_srqDevice.m_busAddrInMemory,
			g_srqDevice.m_busAddrInMemory + sizeof(hess_ring_buffer_t),
			SMC_MEM_START + BASE_HESS1U_DRAWER_READOUT + OFFS_HESS1U_DRAWER_READOUT_EVENTFIFOREADSTROBE,
			HESS_SAMPLE_DATA_SIZE + 2, sizeof(hess_sample_data_debug_t), dev, dmaCallback);
#endif
		
		//AsyncAtDMAreadFast(); //AsyncAtDMAreadSlow();
		
		dev->m_busAddrInCurrent += words*sizeof(hess_sample_data_debug_t);
		if (dev->m_busAddrInCurrent > dev->m_busAddrInMemory + sizeof(hess_ring_buffer_t))dev->m_busAddrInCurrent = dev->m_busAddrInMemory;
		break;
	default: break;
	}
out:
...
static irqreturn_t srqIrqHandlerDMAslow(int irq_number, void *p_dev)
{
	//return srqIrqHandlerDMAfastORslow(irq_number, p_dev, AsyncAtDMAreadSlow);
	//return srqIrqHandlerDMAfastORslow(irq_number, p_dev, AsyncAtDMAreadVectSlow);
	return srqIrqHandlerDMAfastORslow(irq_number, p_dev, AsyncAtDMAreadFifoSlow);
}



static irqreturn_t srqIrqHandlerDMAfast(int irq_number, void *p_dev)
{
	//return srqIrqHandlerDMAfastORslow(irq_number, p_dev, AsyncAtDMAreadFast);
	//return srqIrqHandlerDMAfastORslow(irq_number, p_dev, AsyncAtDMAreadVectFast);
	return srqIrqHandlerDMAfastORslow(irq_number, p_dev, AsyncAtDMAreadFifoFast);
}


static irqreturn_t srqIrqHandlerDMAveryfast(int irq_number, void *p_dev)
{
	//return srqIrqHandlerDMAfastORslow(irq_number, p_dev, AsyncAtDMAreadFast);
	//return srqIrqHandlerDMAfastORslow(irq_number, p_dev, AsyncAtDMAreadVectVeryFast);
	return srqIrqHandlerDMAfastORslow(irq_number, p_dev, AsyncAtDMAreadFifoVeryFast);
}
...
static irqreturn_t srqIrqHandler(int irq_number, void *p_dev)
{
	...
	static hess1u_system_enum hess1u_device_type = HESS1U_SYSTEM_DRAWER;
	...
	case HESS1U_SYSTEM_DRAWER:
		...
		while (words > 0)
		{
			...
			fifo_offset = BASE_HESS1U_DRAWER_READOUT + OFFS_HESS1U_DRAWER_READOUT_EVENTFIFOREADSTROBE;
			for (k = 0; k < (HESS_SAMPLE_DATA_SIZE / 2) + 1; ++k) // ## ist die frage nach was man sich richten sollte... sizeof(structs)?, offsets?, noch mehr defines?
			{
				*(data_pointer++) = smc_bus_read16(fifo_offset); // ## unrolling bringt hier was...
				fifo_offset += 2;
			}
...
static int installSRQHandler(void)
{
	...
	{
	case _NO_DMA_:
		//g_srqDevice.m_pDMAengine = GetAtDMAengineFast();
		//FreeAtDMAengineFast(g_srqDevice.m_pDMAengine);
		g_srqDevice.m_pDMAengine = pDev = NULL;
		break;

	case _SLOW_DMA_:
		g_srqDevice.m_pDMAengine = GetAtDMAengineSlow();
		pDev = g_srqDevice.m_pDMAengine ? GetAtDMAdeviceSlow(g_srqDevice.m_pDMAengine) : NULL;
		break;

	case _FAST_DMA_:
		g_srqDevice.m_pDMAengine = GetAtDMAengineFast();
		pDev = g_srqDevice.m_pDMAengine ? GetAtDMAdeviceFast(g_srqDevice.m_pDMAengine) : NULL;
		break;

	case _VERY_FAST_DMA_:
		g_srqDevice.m_pDMAengine = GetAtDMAengineVeryFast();
		pDev = g_srqDevice.m_pDMAengine ? GetAtDMAdeviceVeryFast(g_srqDevice.m_pDMAengine) : NULL;
		break;
	...
	{
		//m_busfifo_scroll
		g_srqDevice.fifo_scroll_ptr = dma_alloc_coherent(pDev, 2, &(g_srqDevice.m_busfifo_scroll), GFP_KERNEL);
		if (!g_srqDevice.fifo_scroll_ptr)return 1;
		g_srqDevice.hess_data = (hess_ring_buffer_t*)dma_alloc_coherent(pDev, sizeof(hess_ring_buffer_t), &(g_srqDevice.m_busAddrInMemory), GFP_KERNEL);
		g_srqDevice.m_busAddrInCurrent = g_srqDevice.m_busAddrInMemory;
	}
	else
	{
		g_srqDevice.hess_data = (hess_ring_buffer_t*)kmalloc(sizeof(hess_ring_buffer_t), GFP_KERNEL); // ## kmalloc can be an option too
		s_nDMAtype = _NO_DMA_;
	}

	INFO("!!!!!!!!!!!!!!!! g_srqDevice.hess_data = %p\n", g_srqDevice.hess_data);
	...
	//enum _DMA_TYPES_{_NO_DMA_=0,_SLOW_DMA_,_FAST_DMA_};
	switch (s_nDMAtype)
	{
	case _NO_DMA_:
	...
	case _SLOW_DMA_:
		//msleep(10);
		irqRequestResult = request_irq(HESS_IRQ_PIN, srqIrqHandlerDMAslow, IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING, "hess_fpga", &g_srqDevice);
		break;

	case _FAST_DMA_:
		//msleep(50);
		irqRequestResult = request_irq(HESS_IRQ_PIN, srqIrqHandlerDMAfast, IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING, "hess_fpga", &g_srqDevice);
		break;

	case _VERY_FAST_DMA_:
		irqRequestResult = request_irq(HESS_IRQ_PIN, srqIrqHandlerDMAveryfast, IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING, "hess_fpga", &g_srqDevice);
		break;
...
static void uninstallSRQHandler(void)
{
	...
	case _NO_DMA_:break;
	case _SLOW_DMA_:
		StopAtDMAengineSlow(g_srqDevice.m_pDMAengine);
		break;

	case _FAST_DMA_:
		StopAtDMAengineFast(g_srqDevice.m_pDMAengine);
		break;

	case _VERY_FAST_DMA_:
		StopAtDMAengineVeryFast(g_srqDevice.m_pDMAengine);
		break;
	...
	case _SLOW_DMA_:
		pDev = GetAtDMAdeviceSlow(g_srqDevice.m_pDMAengine);
		dma_free_coherent(pDev, sizeof(hess_ring_buffer_t), g_srqDevice.hess_data, g_srqDevice.m_busAddrInMemory);
		dma_free_coherent(pDev, 2, g_srqDevice.fifo_scroll_ptr, g_srqDevice.m_busfifo_scroll);
		FreeAtDMAengineSlow(g_srqDevice.m_pDMAengine);
		break;

	case _FAST_DMA_:
		pDev = GetAtDMAdeviceFast(g_srqDevice.m_pDMAengine);
		dma_free_coherent(pDev, sizeof(hess_ring_buffer_t), g_srqDevice.hess_data, g_srqDevice.m_busAddrInMemory);
		//m_busfifo_scroll
		dma_free_coherent(pDev, 2, g_srqDevice.fifo_scroll_ptr, g_srqDevice.m_busfifo_scroll);
		FreeAtDMAengineFast(g_srqDevice.m_pDMAengine);
	...
	case _VERY_FAST_DMA_:
		pDev = GetAtDMAdeviceVeryFast(g_srqDevice.m_pDMAengine);
		dma_free_coherent(pDev, sizeof(hess_ring_buffer_t), g_srqDevice.hess_data, g_srqDevice.m_busAddrInMemory);
		//m_busfifo_scroll
		dma_free_coherent(pDev, 2, g_srqDevice.fifo_scroll_ptr, g_srqDevice.m_busfifo_scroll);
		FreeAtDMAengineVeryFast(g_srqDevice.m_pDMAengine);
		break;
...
static int __init hess_init(void)
{
	...
	/*pEngine = GetAtDMAengineFast();
	printk(KERN_ALERT "++++++++++++++pEngine = %p\n",pEngine);
	FreeAtDMAengineFast(pEngine);
	return -1;*/

	ret = hess_debugfs_init();
	printk(KERN_ALERT "ln:%d-> ret=%d\n", __LINE__, ret);
	if (ret != 0) return ret;
