#define NUMBER_OF_CHANNELS_IN_USE	3
...
static uint32_t	s_nIterations = 0; // Keep time ~10>-15
...
{ \
	...
	uint32_t timeDiffMult1000000; \
	getnstimeofday(&current_time); \
	timeDiffMult1000000 =	1000000000LU * ((uint64_t)(current_time.tv_sec - a_time_new.tv_sec)) + \
							((uint64_t)(current_time.tv_nsec - a_time_new.tv_nsec)); \
	timeDiffMult1000000 -= a_dev->debug_data.m_nTavNanoSecs; \
	/*do_div(timeDiffMult1000000,++s_nIterations);*/ \
	timeDiffMult1000000 = timeDiffMult1000000 / ++s_nIterations; \
...
static inline void dmaengine_callback(void *a_pDev)
{
	...
	//IRQDevice_t* dev = (IRQDevice_t*)a_pDev;
	IRQDevice_t* dev = &g_srqDevice;
	const int nChannelIndex = (int)a_pDev;
	struct dma_chan* pChan = s_vChannelsB[nChannelIndex];
	...
	if (((type & 0xfc00) == DATATYPE_SAMPLE_CONTROL) || ((type & 0xfc00) == DATATYPE_TESTDATA_FRONTENDFIFOCONTROL))
	{
		...
		s_packet_length = dev->hess_data->write_pointer->as_control.packet_length - 1;
		s_event_length = 0;

		s_event_number_new = (dev->hess_data->write_pointer->as_control.event_counter_h << 16) + dev->hess_data->write_pointer->as_control.event_counter_l;
		dev->debug_data.eventcounter = s_event_number_new;
		if ((uint32_t)(s_event_number_old + 1) != s_event_number_new)
		{
			dev->debug_data.eventcounter_mismatch_old = s_event_number_old;
			dev->debug_data.eventcounter_mismatch_new = s_event_number_new;
	...
	dmaDst = g_dmaDestAddr + (dma_addr_t)((char*)(&(dev->hess_data->write_pointer->as_raw_u16[0])) - (char*)g_srqDevice.hess_data);
...
irqreturn_t IrqHandlerDma(int irq_number, void *p_dev)
{
	...
	//uint16_t words = 0; // ??? or we should set it to zero???
	//uint16_t numberOfEventsInFifo = 0; // ??? or we should set it to zero???
	...
	// words = smc_bus_read16(BASE_NECTAR_ADC + OFFS_EVENT_FIFO_WORD_COUNT); // Is it needed to call it second time???
	...
	dev->debug_data.fifo_count = s_words;
	if (s_words > dev->debug_data.max_fifo_count)
	{
		dev->debug_data.max_fifo_count = s_words;
	}
	...
	//tx->callback = dmatest_callbackIRQ;//dmaengine_callback
	tx->callback = dmaengine_callback;//dmaengine_callback
	//tx->callback_param = dev;
	//tx->callback_param = (void*)(1 % NUMBER_OF_CHANNELS_IN_USE);
	tx->callback_param = (void*)0;
