...
{ \
	...
	*(tmp_slot_numM_ptr) = 	(tmp_slot_numM); \
	while(s_vPci_gens[*(tmp_slot_numM_ptr)].dev_sts && i<PCIE_GEN_NR_DEVS) \
	{ \
...
static int AddUserProgram(struct SDeviceParams* a_params,pointer_type a_user_callback)
{
	...
	for (i = 0; i < PCIE_GEN_NR_DEVS; ++i)
	{
		...
		if (s_vPci_gens[i].dev_sts>0 && 
			_COMPARE_2_IDS6_(a_params, s_vPci_gens[i].pciedev_pci_dev, s_vPci_gens[i].pciedev_pci_dev->class, 0))
		{
			DEBUGNEW("!!!!!!!!!!!!!!!!! Found!\n");
			PrepareSigInfo(&aInfo, &s_vPci_gens[i]);
...
static int __devinit Mtcapciegen_probe(struct pci_dev *a_dev, const struct pci_device_id *a_id)
{
	...
	list_for_each_entry(pDrvData, pHead, m_node)
	{
		...
		if (pDrvData->m_slot_num < 0 || pDrvData->m_slot_num == tmp_slot_num0)
		{
			...
			s_vPci_gens[tmp_slot_numF].pciedev_pci_dev = a_dev;
			s_vPci_gens[tmp_slot_numF].irq_type = /*pDrvData->m_irq_type*/0;
			s_vPci_gens[tmp_slot_numF].slot_num = tmp_slot_num0;
			s_vPci_gens[tmp_slot_numF].dev_minor = PCIE_GEN_MINOR + tmp_slot_numF + 1;
			s_vPci_gens[tmp_slot_numF].m_id = a_id;
			s_vPci_gens[tmp_slot_numF].brd_num = tmp_slot_numF;
			fpProbe = (PROBETYPE)pDrvData->probe;

			if ((*fpProbe)(&s_vPci_gens[tmp_slot_numF], pDrvData->callbackData))
			{
				s_vPci_gens[tmp_slot_numF].dev_sts = 0;
				return -3;
			}
			...
			//	(int)tmp_slot_num0);
			s_vPci_gens[tmp_slot_numF].remove = pDrvData->remove;
			s_vPci_gens[tmp_slot_numF].deviceData = pDrvData->callbackData;
			//s_vPci_gens[tmp_slot_numF].slot_num = tmp_slot_num0;
			//s_vPci_gens[tmp_slot_numF].m_id = a_id;
			...
			InformAllUserPrograms2(&s_vPci_gens[tmp_slot_numF], GAINING_ACCESS);
...
NO_FILE_OK:
	...
	for (i = 0; i < PCIE_GEN_NR_DEVS; ++i) s_vPci_gens[i].dev_sts = 0;
	...
	for (i = 0; i < PCIE_GEN_NR_DEVS; ++i)
	{
		...
		s_vPci_gens[i].slot_num = 0;
		//s_vPci_gens[i].m_PciGen.m_nCreateInsideBigStruct = 1;
		s_vPci_gens[i].m_id = NULL;
		for (j = 0; j < NUMBER_OF_BARS; ++j){ s_vPci_gens[i].memmory_base[j] = 0; }
	}
