...
EXPORT_SYMBOL(upciedev_pciedev_dev_init1_exp);



int CdevInitFor_pciedev_dev(pciedev_dev* a_dev, pciedev_cdev* a_pciedev_cdev_p, struct file_operations *a_pciedev_fops, int a_nBrdNum)
{
	int result;
	int devno = MKDEV(a_pciedev_cdev_p->PCIEDEV_MAJOR, a_pciedev_cdev_p->PCIEDEV_MINOR + a_nBrdNum);
	a_dev->dev_num = devno;
	a_dev->dev_minor = (a_pciedev_cdev_p->PCIEDEV_MINOR + a_nBrdNum);
	cdev_init(&(a_dev->cdev), a_pciedev_fops);
	a_dev->cdev.owner = THIS_MODULE;
	a_dev->cdev.ops = a_pciedev_fops;
	result = cdev_add(&(a_dev->cdev), devno, 1);
	if (result)
	{
		printk(KERN_NOTICE "Error %d adding devno%d num%d\n", result, devno, a_nBrdNum);
		return result;
	}
	return 0;
}
...
int upciedev_init_module_exp(pciedev_cdev **pciedev_cdev_pp, struct file_operations *pciedev_fops, char *a_dev_name)
{
	int                    i = 0;
	int                    k = 0;
	int                    result = 0;
	...
	*pciedev_cdev_pp = pciedev_cdev_p;
	...
	result = alloc_chrdev_region(&devt, pciedev_cdev_p->PCIEDEV_MINOR, (PCIEDEV_NR_DEVS + 1), a_dev_name);
	pciedev_cdev_p->PCIEDEV_MAJOR = MAJOR(devt);
	/* Populate sysfs entries */
	pciedev_cdev_p->pciedev_class = class_create(pciedev_fops->owner, a_dev_name);
	...
	for (i = 0; i <= PCIEDEV_NR_DEVS; i++)
	{
		...
		upciedev_pciedev_dev_init1_exp(pciedev_cdev_p->pciedev_dev_m[i], pciedev_cdev_p, i);

	}

	if (CdevInitFor_pciedev_dev(pciedev_cdev_p->pciedev_dev_m[PCIEDEV_NR_DEVS], pciedev_cdev_p, pciedev_fops, PCIEDEV_NR_DEVS) == 0)
	{
		pciedev_cdev_p->pciedev_dev_m[PCIEDEV_NR_DEVS]->binded = 1;
		pciedev_cdev_p->pciedev_dev_m[PCIEDEV_NR_DEVS]->null_dev = 1;
	}
...
ssize_t pciedev_procinfo(struct file *filp, char * __user buf, size_t count, loff_t *offp)
{

	static char m_p[2048]; /// new
	char *p;
	//char m_p[2048];
