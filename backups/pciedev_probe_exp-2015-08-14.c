...
struct file_operations *pciedev_fops, pciedev_cdev *pciedev_cdev_p, char *dev_name, int * brd_num)
{
	...
	*brd_num = m_brdNum;
	...
	printk(KERN_INFO "PCIEDEV_PROBE:  CREAT DEVICE MAJOR %i MINOR %i F_NAME %s DEV_NAME %s\n", /// new
		pciedev_cdev_p->PCIEDEV_MAJOR, (pciedev_cdev_p->PCIEDEV_MINOR + m_brdNum), f_name, prc_entr); /// new
...
int pciedev_gain_access_exp(struct pci_dev * a_pcidev, struct pciedev_dev* a_dev, struct class* a_class,
	const struct file_operations *a_pciedev_fops, const char *a_dev_name, int a_major, int a_minor)
{
	...
	a_dev->pciedev_pci_dev = a_pcidev;
	a_dev->binded = 1;

	if ((err = pci_enable_device(a_pcidev))) return err;
	err = pci_request_regions(a_pcidev, a_dev_name);
	if (err)
	{
		pci_disable_device(a_pcidev);
		pci_set_drvdata(a_pcidev, NULL);
		a_dev->binded = 0;
		a_dev->pciedev_pci_dev = NULL;
		return err;
	}
	pci_set_master(a_pcidev);

	tmp_devfn = (u32)a_pcidev->devfn;
	busNumber = (u32)a_pcidev->bus->number;
	devNumber = (u32)PCI_SLOT(tmp_devfn);
	funcNumber = (u32)PCI_FUNC(tmp_devfn);
	tmp_bus_func = ((busNumber & 0xFF) << 8) + (devNumber & 0xFF);
	printk(KERN_ALERT "PCIEDEV_PROBE:DEVFN %X, BUS_NUM %X, DEV_NUM %X, FUNC_NUM %X, BUS_FUNC %x\n",
	...
	tmp_devfn = (u32)a_pcidev->bus->self->devfn;
	busNumber = (u32)a_pcidev->bus->self->bus->number;
	devNumber = (u32)PCI_SLOT(tmp_devfn);
	funcNumber = (u32)PCI_FUNC(tmp_devfn);
	printk(KERN_ALERT "PCIEDEV_PROBE:DEVFN %X, BUS_NUM %X, DEV_NUM %X, FUNC_NUM %X\n",
		tmp_devfn, busNumber, devNumber, funcNumber);

	pcie_cap = pci_find_capability(a_pcidev->bus->self, PCI_CAP_ID_EXP);
	printk(KERN_INFO "PCIEDEV_PROBE: PCIE SWITCH CAP address %X\n", pcie_cap);

	pci_read_config_dword(a_pcidev->bus->self, (pcie_cap + PCI_EXP_SLTCAP), &tmp_slot_cap);
	tmp_slot_num = (tmp_slot_cap >> 19);
	tmp_dev_num = tmp_slot_num;
	printk(KERN_ALERT "PCIEDEV_PROBE:SLOT NUM %d DEV NUM%d SLOT_CAP %X\n", tmp_slot_num, tmp_dev_num, tmp_slot_cap);

	a_dev->swap = 0;
	a_dev->slot_num = tmp_slot_num;
	a_dev->bus_func = tmp_bus_func;
	a_dev->pciedev_pci_dev = a_pcidev;

	dev_set_drvdata(&(a_pcidev->dev), a_dev);

	pcie_cap = pci_find_capability(a_pcidev, PCI_CAP_ID_EXP);
	printk(KERN_ALERT "DAMC_PROBE: PCIE CAP address %X\n", pcie_cap);
	pci_read_config_byte(a_pcidev, (pcie_cap + PCI_EXP_DEVCAP), &dev_payload);
	dev_payload &= 0x0003;
	printk(KERN_ALERT "DAMC_PROBE: DEVICE CAP  %X\n", dev_payload);
	...
	if (!(cur_mask = pci_set_dma_mask(a_pcidev, DMA_BIT_MASK(64))) &&
		!(cur_mask = pci_set_consistent_dma_mask(a_pcidev, DMA_BIT_MASK(64)))) {
		a_dev->dev_dma_64mask = 1;
		printk(KERN_ALERT "CURRENT 64MASK %i\n", cur_mask);
	}
	else {
		if ((err = pci_set_dma_mask(a_pcidev, DMA_BIT_MASK(32))) &&
			(err = pci_set_consistent_dma_mask(a_pcidev, DMA_BIT_MASK(32)))) {
			printk(KERN_ALERT "No usable DMA configuration\n");
		}
		else{
	...
	pci_read_config_word(a_pcidev, PCI_VENDOR_ID, &vendor_id);
	pci_read_config_word(a_pcidev, PCI_DEVICE_ID, &device_id);
	pci_read_config_word(a_pcidev, PCI_SUBSYSTEM_VENDOR_ID, &subvendor_id);
	pci_read_config_word(a_pcidev, PCI_SUBSYSTEM_ID, &subdevice_id);
	pci_read_config_word(a_pcidev, PCI_CLASS_DEVICE, &class_code);
	pci_read_config_byte(a_pcidev, PCI_REVISION_ID, &revision);
	pci_read_config_byte(a_pcidev, PCI_INTERRUPT_LINE, &irq_line);
	pci_read_config_byte(a_pcidev, PCI_INTERRUPT_PIN, &irq_pin);
	...
	a_dev->vendor_id = vendor_id;
	a_dev->device_id = device_id;
	a_dev->subvendor_id = subvendor_id;
	a_dev->subdevice_id = subdevice_id;
	a_dev->class_code = class_code;
	a_dev->revision = revision;
	a_dev->irq_line = irq_line;
	a_dev->irq_pin = irq_pin;
	a_dev->scratch_bar = 0;
	a_dev->scratch_offset = 0;

	/*******SETUP BARs******/
	a_dev->pciedev_all_mems = 0;
	for (i = 0, nToAdd = 1; i < NUMBER_OF_BARS; ++i, nToAdd *= 2)
	{
		res_start = pci_resource_start(a_pcidev, i);
		res_end = pci_resource_end(a_pcidev, i);
		res_flag = pci_resource_flags(a_pcidev, i);
		a_dev->mem_base[i] = res_start;
		a_dev->mem_base_end[i] = res_end;
		a_dev->mem_base_flag[i] = res_flag;
		if (res_start){
			a_dev->memmory_base[i] = pci_iomap(a_pcidev, i, (res_end - res_start));
			printk(KERN_INFO "PCIEDEV_PROBE: mem_region %d address %X  SIZE %X FLAG %X\n",
				i, res_start, (res_end - res_start),
				a_dev->mem_base_flag[i]);
			a_dev->rw_off[i] = (res_end - res_start);
			a_dev->pciedev_all_mems += nToAdd;
		}
		else{
			a_dev->memmory_base[i] = 0;
			a_dev->rw_off[i] = 0;
			printk(KERN_INFO "PCIEDEV: NO BASE%i address\n", i);
		}
	}
	...
	/******GET BRD INFO******/
	tmp_info = pciedev_get_brdinfo(a_dev);
	printk(KERN_ALERT "$$$$$$$$$$$$$PROBE  IS STARTUP BOARD %i\n", tmp_info);
	if (tmp_info){
		tmp_info = pciedev_get_prjinfo(a_dev);
		printk(KERN_ALERT "$$$$$$$$$$$$$PROBE  NUMBER OF PRJs %i\n", tmp_info);
	}
	/*******PREPARE INTERRUPTS******/

	a_dev->irq_flag = IRQF_SHARED | IRQF_DISABLED;
#ifdef CONFIG_PCI_MSI
	if (pci_enable_msi(a_pcidev) == 0) {
		a_dev->msi = 1;
		a_dev->irq_flag &= ~IRQF_SHARED;
		printk(KERN_ALERT "MSI ENABLED\n");
	}
	else {
		a_dev->msi = 0;
...
#endif
	a_dev->pci_dev_irq = a_pcidev->irq;
	a_dev->irq_mode = 0;

	/* Send uvents to udev, so it'll create /dev nodes */
	if (a_dev->dev_sts){
		a_dev->dev_sts = 0;
		device_destroy(a_class, MKDEV(a_major, a_minor));
	}
	a_dev->dev_sts = 1;
	sprintf(a_dev->dev_name, "%ss%d", a_dev_name, tmp_slot_num);
	sprintf(f_name, "%ss%d", a_dev_name, tmp_slot_num);
	sprintf(prc_entr, "%ss%d", a_dev_name, tmp_slot_num);
	//printk(KERN_INFO "PCIEDEV_PROBE:  CREAT DEVICE MAJOR %i MINOR %i F_NAME %s DEV_NAME %s\n",
	//		pciedev_cdev_p->PCIEDEV_MAJOR, (pciedev_cdev_p->PCIEDEV_MINOR + m_brdNum), f_name);

	printk(KERN_INFO "PCIEDEV_PROBE:  CREAT DEVICE MAJOR %i MINOR %i F_NAME %s DEV_NAME %s\n", /// new
		a_major, a_minor, f_name, prc_entr); /// new

	device_create(a_class, NULL, MKDEV(a_major, a_minor),
		&a_dev->pciedev_pci_dev->dev, f_name, a_dev->dev_name);
...
EXPORT_SYMBOL(pciedev_gain_access_exp);
